    ...
    if (success) {
      ...
      data.forEach((item) => {
        switch (item.key) {
          ...
          case 'discord.guild_verify_enabled':
          case 'oidc.enabled':
              ...
              <Card>
                <Form.Section text={t('配置 Discord OAuth')}>
                  ...
                  >
                    <Col xs={24} sm={24} md={24} lg={24} xl={24}>
                      <Form.Checkbox
                        field="['discord.guild_verify_enabled']"
                        noLabel
                        onChange={(e) =>
                          handleCheckboxChange('discord.guild_verify_enabled', e)
                        }
                      >
                        {t('启用 Discord 服务器验证')}
                      </Form.Checkbox>
                      <Text type='secondary' style={{ display: 'block', marginTop: 4 }}>
                        {t('开启后，用户必须加入指定服务器并拥有指定身份组才能注册/登录')}
                      </Text>
                    </Col>
                  </Row>
                  <Row
                  ...
                  >
                    <Col xs={24} sm={24} md={12} lg={12} xl={12}>
                      <Form.Input
                        ...
                        label={t('必须加入的服务器 ID')}
                        placeholder={t('输入 Discord 服务器 ID')}
                        extraText={t('用户必须是该服务器的成员才能注册/登录')}
                        disabled={!inputs['discord.guild_verify_enabled']}
                      />
                    </Col>
                    <Col xs={24} sm={24} md={12} lg={12} xl={12}>
                      <Form.Input
                        ...
                        label={t('必须拥有的身份组 ID')}
                        placeholder={t('多个 ID 用逗号分隔')}
                        extraText={t('用户必须拥有其中至少一个身份组才能注册/登录，留空则不检查身份组')}
                        disabled={!inputs['discord.guild_verify_enabled']}
                      />
                    </Col>
                  </Row>
